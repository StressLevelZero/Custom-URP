Shader "SLZ/Particle/Motion Vector Billboard Correct Shadows"
{
	...
	SubShader
	{
		...
		// 		float3 normal84 = v.ase_normal;
		// 		float3 tangent84 = float3( 0,0,0 );
		// 		float3 appendResult88 = (float3(v.texcoord1.z , v.texcoord1.w , v.texcoord2.x));
		// 		float3 center84 = appendResult88;
		// 		float3 localparticle_face_camera84 = particle_face_camera( vertex84 , normal84, center84 );
		...
		// 		float2 temp_output_8_0 = (IN.ase_texcoord4).xy;
		// 		float4 temp_cast_1 = (0.5).xxxx;
		// 		float2 temp_output_9_0 = (IN.ase_texcoord4).zw;
		// 		float4 temp_cast_4 = (0.5).xxxx;
		// 		float4 lerpResult4 = lerp( tex2D( _MainTex, ( float4( temp_output_8_0, 0.0 , 0.0 ) + ( ( tex2D( _MotionVectors, temp_output_8_0 ) - temp_cast_1 ) * IN.ase_texcoord5.xyz.x * -_UVMotionMultiplier ) ).rg ) , tex2D( _MainTex, ( float4( temp_output_9_0, 0.0 , 0.0 ) + ( ( tex2D( _MotionVectors, temp_output_9_0 ) - temp_cast_4 ) * ( 1.0 - IN.ase_texcoord5.xyz.x ) * -1.0 * -_UVMotionMultiplier ) ).rg ) , IN.ase_texcoord5.xyz.x);
		// 		float4 temp_output_11_0 = ( IN.ase_color * lerpResult4 * _Color );
